...
@Generated
public interface ReactiveHttpClientUtils extends HttpClientHandlers {
    ...
    Predicate<HttpStatusCode> isTooManyRequests = HttpStatus.TOO_MANY_REQUESTS::equals;
    ...
    private InternalErrorException handleWebClientException(
        ...
        if (cause instanceof ConnectException) {
            logger().info("ConnectionTimeout error observed from {} [resourceId:{}]", namespace(), id);
        ...
        if (cause instanceof ReadTimeoutException || cause instanceof WriteTimeoutException) {
            logger().info("Timeout error observed from {} [resourceId:{}]", namespace(), id);
            return InternalErrorException.with("Timeout error observed from %s [resourceId:%s]"
                    .formatted(namespace(), id));
        }

        logger().info("Error observed from {} [resourceId:{}]", namespace(), id, ex);
    ...
    private InternalErrorException handleThrowable(final String id, final Throwable t) {
        ...
        logger().error("Unhandled error observed from {} [resourceId:{}]", namespace(), id, t);
    ...
    private InternalErrorException handleWebClientException(final WebClientException ex) {
        ...
        if (cause instanceof ConnectException) {
            logger().info("ConnectionTimeout error observed from {} on making request", namespace());
            return InternalErrorException.with("ConnectionTimeout error observed from %s on making request"
                    .formatted(namespace()));
        }

        if (cause instanceof ReadTimeoutException || cause instanceof WriteTimeoutException) {
            logger().info("Timeout error observed from {} on making request", namespace());
            return InternalErrorException.with("Timeout error observed from %s on making request"
                    .formatted(namespace()));
        }

        logger().info("Error observed from {} on making request", namespace(), ex);
    ...
    private InternalErrorException handleThrowable(final Throwable t) {
        ...
        logger().error("Unhandled error observed from {} on making request", namespace(), t);
